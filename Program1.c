...
void print_the_data(const char *months[], const float *sales){
	printf("Month\tSales\n");
	for(int i = 0; i < MONTHS; ++i){
		printf("%-8s\t$%.2f\n",months[i], sales[i]);
...
void print_the_summary(const char *months[], const float *sales){
	printf("\nSales Summary:\nMinimum Sales: ");
	float min = sales[0];
	for(int i = 0; i < MONTHS; ++i){
		if(sales[i] < min){
...
void print_the_moving_avg(const char *months[], const float *sales){
	printf("\nSix-Month Moving Average Report:\n");
	int start = 0;
	int end = 5;
	float total = 0;
	while(start < 7){
		printf("%s             ", months[start], months[end]);
...
int main(){
	...
	print_the_data(months, sales);
	print_the_summary(months, sales);
	print_the_moving_avg(months, sales);
